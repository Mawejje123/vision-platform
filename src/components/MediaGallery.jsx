...
const MediaGallery = ({ project }) => {
  ...
  const hasMedia = project.image || project.gallery?.length > 0 || project.videoLink || project.demoLink;
  ...
  <div>
    <h3 className="text-xl font-bold text-white mb-4 flex items-center space-x-2">
      <ImageIcon size={20} className="text-orange-500" />
      <span>Gallery ({allImages.length} {allImages.length === 1 ? 'image' : 'images'})</span>
    </h3>
